  ...
  els.captainsList.innerHTML = captains.length ? captains.map((captain) => `
    <div class="captain-review" data-id="${captain.id}">
      ...
      <div class="document-grid">
        ${Object.keys(docLabels).map((key) => `<label class="document-upload"><span>${docLabels[key]}</span><input type="file" accept="image/*" capture="environment" data-doc="${key}" /><img src="${captain.documents?.[key] || ''}" data-preview="${key}" /></label>`).join('')}
      </div>
