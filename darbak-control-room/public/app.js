...
function renderCaptains(captains) {
  ...
  els.captainsList.innerHTML = captains.length ? captains.map((captain) => `
    <div class="captain-review" data-id="${captain.id}">
      ...
      <div class="grid"><label class="field"><span>نوع السيارة</span><input data-vehicle="carType" value="${captain.vehicle?.carType || ''}" /></label><label class="field"><span>رقم السيارة</span><input data-vehicle="carNumber" value="${captain.vehicle?.carNumber || ''}" /></label><label class="field"><span>سعة السيارة</span><input data-vehicle="capacity" value="${captain.vehicle?.capacity || ''}" /></label></div>
